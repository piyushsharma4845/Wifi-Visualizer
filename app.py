...
# --- 1. SMART HARDWARE DETECTION (Local vs Cloud) ---
def get_wifi_details():
    """Detects if running on Cloud or Local Hardware and returns signal data"""
    os_name = platform.system()
    
    # 1. Check if we are on a Cloud Server (like Streamlit Cloud)
    # Servers usually use Linux but don't have Wi-Fi cards/nmcli
    is_cloud = False
    if os_name == "Linux":
        try:
    ...
    if is_cloud:
        # WEB DEMO MODE: Returns random signal for users to try the app
        return "Demo_Network_5G", np.random.randint(65, 95)
    ...
    try:
        ...
        # FALLBACK: If hardware access is denied
        return "Simulation_Mode", np.random.randint(40, 90)
    
    return "Unknown_Host", 0

# --- 2. PHYSICS ENGINE ---
def rssi_to_meters(signal_pct):
    """Converts Signal % to distance with snapping logic for accuracy"""
...
with col_sidebar:
    ...
    # Show progress of the 3-point scan
    progress = len(st.session_state.points) / 3
    st.progress(progress)
    
    st.markdown("**Move sliders to your current spot:**")
    ...
    if st.button("📌 Log Location Data"):
        if len(st.session_state.points) < 3:
            ...
            if len(st.session_state.points) == 3:
                # Weighted Trilateration Calculation
    ...
    for i, p in enumerate(st.session_state.points):
        st.caption(f"P{i+1}: Location({p[0]}, {p[1]}) | Dist: {p[2]}m")
...
with col_map:
    ...
    # Use calibrated pos or stay at center
    ...
    ax.set_xlim(0, grid)
    ax.set_ylim(0, grid)
    ax.axis('off') # Cleaner radar look
